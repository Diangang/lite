...
### Gate 0. Ledger-First Hard Stop（硬闸门：不出账本不许改）

本闸门用于解决“口头说穷举但实际漏项”的问题：**任何编辑之前必须先交付可复核的全量账本**，并得到用户确认。

**适用范围（用户显式触发后，无条件执行）：**
- 只要用户明确说“使用 linux-alignment / 使用 Kernel 对齐 skill”并指定范围（目录/文件/子系统/模块），Gate 0 必须对该范围无条件执行。
- 禁止以“范围太大”“时间不够”“我先改主流程”“这是实现简化”为由跳过或缩减账本。
- 若执行中范围扩展（例如用户追加目录/文件/模块），必须先回到 Gate 0，为新增范围补齐账本并等待用户确认。

**硬性交付物：Mapping Ledger（必须在编辑前给出）**
- `Files`：目标目录下所有文件（`*.c/*.h/Makefile/Kconfig/README` 等）完整列表。
- `Per-file symbols`：对每个文件列出：
  - `functions`：所有函数定义（static/非static都算）
  - `structs`：所有 `struct` 定义
  - `globals/statics`：所有文件作用域变量（全局/静态）
  - `exported`：所有导出符号（如有）
- `Linux mapping`：对每个符号给出 Linux2.6 参考树中的对应项：
  - 能对应：写 `linux2.6/<path>::<symbol>`（至少 1 个）
  - 不能对应：必须标 `NO_DIRECT_LINUX_MATCH`，并写 `Why/Impact/Plan`

**强制流程：**
1. 先用工具生成 `Files` 和 `Per-file symbols`（禁止仅凭“主流程阅读”）。
2. 先给用户输出 Ledger，让用户确认范围/接受 `NO_DIRECT_LINUX_MATCH` 项。
3. **用户未明确确认前，禁止做任何代码编辑**（包括重命名、移动文件、删除文件、改逻辑）。

**违反处理（必须执行）：**
- 若发现漏项或对齐后用户指出“你没检查到 X”：立即停止编辑，回到 Gate 0 重新出账本；不得辩解为“实现简化”。
...
> 注意：Gate 5 仅描述“范围”应包含什么；**Gate 0** 才是强制的“交付形式/硬闸门”，两者缺一不可。
