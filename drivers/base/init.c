...
{
    /* ksets must exist before any static bus/class/driver registration that uses kset_add(). */
    ...
    struct device *root = device_register_simple("platform", "platform", &platform_bus, NULL);
    if (root)
