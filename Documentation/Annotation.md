...
*   **`isr_common_stub` / `irq_common_stub`**
    ...
    *   **传递参数**：`push %esp`，将当前栈指针（即 `struct registers` 结构体的起始地址）作为参数传给 C 函数。
...
*   **`keyboard_callback(struct registers *regs)`**
...
*   **`init_pmm`**：读取 Multiboot 结构体，获取 `mem_lower` (低端内存) 和 `mem_upper` (高端内存) 的大小。
    *   **详细流程**：
        ...
        3.  对齐位图起始地址到页边界，计算位图大小（`total_pages / 32`，不足则向上取整）。
            *   **为什么是 `/32`**：位图用 `uint32_t` 数组表示，每个 `uint32_t` 有 32 个 bit，能覆盖 32 个物理页帧（1 bit 对应 1 页）。
            *   `bitmap_size` 表示“需要多少个 `uint32_t` 元素”，因此位图占用字节数为 `bitmap_size * sizeof(uint32_t)`。
        4.  计算 `pmm_refcount` 的放置位置，并初始化元数据：
            *   `pmm_refcount` 是 `uint8_t` 数组，下标就是物理页帧号 `frame`，用于记录该物理页被引用/共享的次数（例如 fork+COW 或共享页）。
            *   为了让 `pmm_refcount` 紧跟在位图之后，代码使用“字节指针”做偏移：`(uint8_t*)pmm_bitmap + bitmap_size * sizeof(uint32_t)`，得到位图末尾的字节地址，再强转为 `uint8_t*` 作为 refcount 表起始地址。
            *   `refcount_size = total_pages * sizeof(uint8_t)` 表示 refcount 表占用的总字节数。
            *   最后初始化位图为全 1（默认全部占用），refcount 清零；再根据 BIOS E820 内存地图逐步释放可用区域。
...
*   **mm 结构**：
    *   每个任务持有独立的 `mm`，包含 `page_directory` 与 `vma_list/heap/stack` 等地址空间元信息，用于缺页校验与回收。
...
*   **`isr_handler` / `irq_handler` 调用流程**：
    *   **异常路径（ISR）**：
        ...
        *   `isr_common_stub` 保存现场、切换段寄存器，把 `struct registers*` 传给 `isr_handler`。
...
*   **Multiboot 模块**：
    *   GRUB 通过 `struct multiboot_info` 结构体中的 `mods_addr` 和 `mods_count` 字段告诉内核 InitRD 在内存的哪个位置。
    *   **关键坑点**：`mods_addr` 指向的是 `struct multiboot_module` 结构体数组，而不是文件数据本身！必须通过 `mod->mod_start` 获取文件内容的物理地址。
*   **`kernel_main` 中的 InitRD 初始化流程**：
    ...
    *   调用 `init_initrd(initrd_location)` 解析文件系统，构建 `initrd_root`。
...
*   **文件系统解析**：
    *   我们定义了一个极简的自定义文件系统格式（由 `mkinitrd.c` 生成）：
        ...
        *   最后是文件数据。
    *   `init_initrd`：解析内存中的文件头，构建文件节点树 (`struct vfs_inode`)。
...
*   **`init_initrd` 关键流程**：
    ...
    *   为每个文件创建 `struct vfs_inode` 节点，设置 `read` 回调并将偏移转换为内存绝对地址。
...
面向后续 Linux-like 方向的最新路线图见 [docs/roadmap_v4.md](./docs/roadmap_v4.md)。
...
*   **设备模型（kobject/device/driver/bus）**：
    ...
    *   当前默认注册 console/initrd/ramfs，并用同名 driver 自动绑定用于验证 probe/bind 框架。
*   **VFS（对象化进行中）**：
    ...
    *   引入 ramfs 作为可写根：`/` 指向 ramfs，initrd 改挂到 `/initrd`，并默认把 cwd 切到 `/initrd` 以保持 `run user.elf` 等习惯用法。
*   **系统调用最小闭环**：
...
*   **用户态程序加载**：
    *   `run <file>` 从 InitRD 读取指定 ELF（如 `run user.elf`），解析 PT_LOAD 段后映射到用户空间并进入执行。
    ...
    *   程序路径解析基于 VFS mount 表（例如 `/proc` `/dev` `/sys` 的路径不会与 initrd 文件混淆）。
    *   进入用户态后切换输入前台为 `user>`，用户任务退出后自动恢复 `lite-os>`。
*   **用户态异常策略**：
    *   用户态触发 CPU 异常（如 `#GP/#UD`）或不可恢复的 `#PF` 时，不再触发内核 `panic`，而是结束当前用户任务并返回 shell。
*   **用户进程退出回收**：
    *   用户任务退出后进入 `ZOMBIE` 状态，记录退出码与退出原因，等待 `task_wait(pid, ...)` 回收资源。
    *   `task_wait` 会使调用者进入 `BLOCKED` 状态，直到目标任务退出事件发生再被唤醒。
*   **kthread 与 mm 语义**：
...
*   **等待队列 (waitqueue)**：
    ...
    *   当前用于：`task_wait` 等待用户任务退出事件，以及 shell 输入到达时唤醒阻塞读者。
*   **独立页表注意点**：
...
### 核心机制与入口
- **`__define_initcall` 宏**：在 `include/init.h` 中，我们利用 GCC 的 `__attribute__((__section__(".initcall.init")))`，将所有驱动的初始化函数指针（例如被 `module_init(keyboard_driver_init)` 标记的函数）集中编译到同一个 ELF 段中。
...
此外，我们还引入了 `struct dentry` 来表示目录项缓存树（dcache），并将 `struct file` 与 `dentry` 强绑定。根目录的挂载也进行了精简，`vfs_root_dentry` 会在第一次挂载时直接分配好。`vfs_mount_root` 负责将设备根节点挂载到指定路径，并为其分配 `struct vfsmount`。
...
- **`ramfs_unlink` 与 内存回收**：
  - 通过 `finddir` 找到目标子节点。
  - 调用 `truncate_inode_pages(inode->i_mapping, 0)` 遍历并释放该文件分配的所有物理页面（`pmm_free_page`）。
