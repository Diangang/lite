...
*   **初始化流程（start_kernel）**：
    *   先完成 `init_mm()` 建立恒等映射与内存分配器，保证模块内存可读。
    *   `vfs_init()` 挂载 `rootfs`（ramfs）成为 `/`。
    *   调用 `populate_rootfs(mbi)` 将 initramfs 解包到 `/`。
*   **解包语义**：
...
1.  **initramfs Triple Fault**：由于错误解引用 Multiboot 结构体指针，导致访问无效地址 `0xFFFFFFFF`，触发缺页异常并升级为 Triple Fault。解决方法是修正指针算术并确保 PMM/VMM 正确映射了模块内存区域。
