# Stage 5 Review: proc status adds PPid

Patch: `stage5-proc-status-add-ppid`
...
- Linux symbols: status rendering, `task_ppid_nr`
- Lite file: `fs/proc/array.c`
- This step only changes: add the Linux `PPid` status field using Lite's existing Linux-shaped `task_ppid_nr()` helper.
...
Consistency:
- Naming: OK, status field uses Linux `PPid`.
- Placement: OK for the proc status surface.
- Semantics: OK, value is parent PID or 0 when unavailable.
...
- `git show -- fs/proc/array.c state.json Documentation/reviews/2026-04-30-stage5-proc-status-add-ppid.md`
