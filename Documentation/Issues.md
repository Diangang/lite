...
## 文档定位
- 这是一份**历史问题日志**，保留的是开发过程中遇到的问题、定位思路和修复过程。
- 其中很多条目描述的是**历史故障现场**，不表示这些问题在当前代码中仍然存在。
- 需要了解当前实现时，请优先查阅 `QA.md` 和对应源码。
...
### 7.1 `cd` 后 `pwd` 输出为单斜杠 `/`
- **现象**：将 `struct task_struct` 中的字符串 `cwd` 替换为 `struct dentry *cwd` 后，进入 `initrd` 目录执行 `pwd` 仍然只输出 `/`。
- **定位**：
  1. `vfs_mount` 挂载文件系统时，给新挂载的根 `dentry` 起名为 `/`，导致它误以为自己是全局树根。
  2. `task_get_cwd` 中的逆向字符串拼接逻辑存在内存覆盖和截断 Bug。
- **解决**：挂载点根 `dentry` 的名字直接继承被覆盖挂载点的名字（如 `initrd`）。重写 `pwd` 逆向构建算法，从缓冲区末尾向前逐个追加目录名，避免内存移动操作。

### 7.2 根目录下 `ls` 打印双倍目录项
- **现象**：`ls /` 时，`sys`, `dev`, `proc`, `initrd` 各出现两次。
- **定位**：引入真正的 Dcache 后，在 `vfs_mount` 时，调用 `d_alloc(d->parent)` 将新文件系统的根节点也加入了原父目录的 `children` 链表中。由于底层还有一个作为挂载锚点的空文件夹存在于该链表中，导致遍历时输出了两份。
- **解决**：挂载的根节点应独立成树。在分配时使用 `d_alloc(NULL)` 防止其混入父目录的链表，仅手动设置 `mount_root->parent = d->parent` 以便 `cd ..` 回退。

### 7.3 用户态 `ls` 没有任何输出
- **现象**：重构 VFS 结构体名称后（将 `vfs_inode` 改为 `inode`），执行用户态的 `ls` 无任何输出。
