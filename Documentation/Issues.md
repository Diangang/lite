...
### 7.1 `cd` 后 `pwd` 输出为单斜杠 `/`
- **现象**：将 `task_t` 中的字符串 `cwd` 替换为 `struct dentry *cwd` 后，进入 `initrd` 目录执行 `pwd` 仍然只输出 `/`。
- **定位**：
  1. `vfs_mount_root` 挂载文件系统时，给新挂载的根 `dentry` 起名为 `/`，导致它误以为自己是全局树根。
...
- **定位**：引入真正的 Dcache 后，在 `vfs_mount_root` 时，调用 `d_alloc(d->parent)` 将新文件系统的根节点也加入了原父目录的 `children` 链表中。由于底层还有一个作为挂载锚点的空文件夹存在于该链表中，导致遍历时输出了两份。
