...
## 文档定位
- 这是 **当前设备驱动模型** 的权威说明文档。
- 它重点解释 `device / driver / bus / class / kobject / sysfs / devtmpfs` 在当前代码中的实际语义。
...
- `serial8250`（平台串口设备，代表 8250/16550A UART）

而更高层的用户语义设备由各自子系统创建：
- `console`：由 console 子系统注册为 `console` class 设备
- `tty`：由 tty 子系统注册为 `tty` class 设备
- `ttyS0`：由 serial8250/tty 子系统通过最小 `tty_driver` 路径注册为 `tty` class 设备，并挂在 `serial8250` 之下
...
| `tty_init()` | `device_initcall` | 合理 | 更接近 Linux | 初始化 tty core 状态并创建 `/dev/tty`，职责对齐 Linux `tty_io.c:tty_init()` |
| `serial8250_driver_init()` | `module_init`=`device_initcall` | 合理 | 合理 | 具体 8250 串口驱动，依赖 tty/class/平台设备已就绪 |
...
- [devtmpfs_create_node](file:///data25/lidg/lite/drivers/base/devtmpfs.c)

设备移除时则是近似反向顺序：

1. `device_unbind()`
2. 从 bus 私有设备成员表 / `class->devices` 脱链
3. `devtmpfs_delete_node()`
4. `uevent remove`
5. 从 `devices_kset` 移除
6. `kobject_put()`
...
- `/sys/bus`
  - 直接遍历 `buses_kset`
  - 每个 bus 目录下面的 `devices/` 来自 bus 私有层中的 `devices_kset + klist_devices`
...
- `/dev`
  ...
  - 只在 `device_add()` 之后，由 `devtmpfs_create_node(dev)` 基于 `device` 本身的信息创建
  - 当前实现已收敛到 `drivers/base/devtmpfs.c`，create/delete/mount 对外边界与 Linux 一样走 `linux/device.h`
  - Lite 的 `prepare_namespace()` 通过 `devtmpfs_mount("/dev")` 挂载，底层仍复用 `ramfs_fill_super()`
  - 参考：[devtmpfs.c](file:///data25/lidg/lite/drivers/base/devtmpfs.c)
...
- `tty`
  ...
  - 参考：[tty_io.c](file:///data25/lidg/lite/drivers/tty/tty_io.c)
...
- tty 输出分发（当前输出到串口）：[tty_io.c](file:///data25/lidg/lite/drivers/tty/tty_io.c)
