...
### 1.1 固定/保留区域
- `0x00000000 ~ 0x000FFFFF`：低端保留（BIOS/实模式遗留区/VGA 等），PMM 初始化时默认不释放此范围。
...
### 1.3 可用页释放原则
PMM 初始化时先把所有页标为“已占用”，然后遍历 BIOS E820 map 仅释放类型为 `MULTIBOOT_MEMORY_AVAILABLE` 且不与内核/元数据冲突的物理页。
...
### 2.2 内核堆 (3GB 起的虚拟映射)
内核堆虚拟基址固定在 `KHEAP_START = 0xC0000000`。
- `kmalloc` 动态向后分配，每次从 PMM 取页并映射到该虚拟地址区间。
- 这个地址作为内核态与用户态的一个重要软边界（用户态指针不得超过此边界）。

### 2.3 Page Cache (文件系统页缓存)
- 新增的文件系统 Page Cache 直接使用 `pmm_alloc_page()` 获取物理页。
...
### 3.1 用户态上限
- 用户态最高可用虚拟地址为 `0xC0000000`。所有系统调用传递的用户指针均需在此边界下。
...
### 3.3 用户栈与堆
- **栈 (Stack)**：栈底位于 `0xBFFFF000`，向下生长。初始 `esp` 设为 `0xC0000000`。
- **堆 (Heap/BRK)**：紧接在 ELF 的 `.bss` 段之后（`heap_base`）。通过 `brk()` 系统调用向上扩展。

### 3.4 匿名映射 (mmap)
- `mmap` 的选址策略为从 `align_up(heap_brk)` 开始向上寻找空闲的 VMA，最高不超过用户栈底。
