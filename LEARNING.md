...
*   **系统调用最小闭环**：
    ...
    *   通过 `SYS_OPEN/SYS_FREAD/SYS_CLOSE` 提供最小只读文件访问能力（基于 InitRD），fdtable 为每任务独立。
    *   `SYS_READFD/SYS_WRITEFD` 提供最小 `read(fd,...)` / `write(fd,...)` 风格接口（fd=0/1/2 为控制台），便于后续对齐 VFS 的 `read/write` 语义。
