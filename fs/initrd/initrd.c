...
struct inode *init_initrd(struct multiboot_info* mbi)
{
    ...
    if (!vmm_is_mapped((void*)location))
        panic("DEBUG: ERROR: InitRD location is not mapped!\n");
    ...
    if (nroot_nodes == 0 || nroot_nodes > 256) {
        printf("DEBUG: ERROR: Invalid InitRD node count: %d\n", nroot_nodes);
        panic("Invalid InitRD node count.");
    }
